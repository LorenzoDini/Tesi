...
void measure_c_re (float c[], float spin_q_re[], float spin_q_im[], float q2[], float spin_re[], float spin_im[], int nmeas, int nop, int nflav) {
	...
	for(iflav=0;iflav<nflav;iflav++)
	    for(i_gauge=0;i_gauge<2;i_gauge++){
/*
		cout << "indici mentre calcolo c_re: imeas=" << imeas << " " << "i_gauge=" << i_gauge << " " << "iflav=" << iflav << endl; 
		cout << "(dentro measure_c) spin_q_re=" << spin_q_re[index_tens_med(imeas,nmeas,3,nop,iflav,nflav,i_gauge)] << endl;
		cout << "(dentro measure_c)spin_q_im=" << spin_q_im[index_tens_med(imeas,nmeas,3,nop,iflav,nflav,i_gauge)] << endl;
		cout << "(dentro measure_c)spin_re=" << spin_re[index_tens_med(imeas,nmeas,2,nop,iflav,nflav,i_gauge)] << endl;
		cout << "(dentro measure_c)spin_im=" << spin_im[index_tens_med(imeas,nmeas,2,nop,iflav,nflav,i_gauge)] << endl;
		cout << "(dentro measure_c)q2=" << q2[index_q_med(imeas,nmeas,i_gauge)] << endl;
		cout << "(dentro measure_c)sqrt(q2)=" << pow(q2[index_q_med(imeas,nmeas,i_gauge)],0.5) << endl;
		cout << "(dentro measure_c)spin_re^2=" << pow(spin_re[index_tens_med(imeas,nmeas,2,nop,iflav,nflav,i_gauge)],2) << endl;
		cout << "(dentro measure_c)spin_im^2=" << pow(spin_im[index_tens_med(imeas,nmeas,2,nop,iflav,nflav,i_gauge)],2) << endl;
*/
...
for(imeas=0;imeas<nmeas;imeas++)
	for(iflav=0;iflav<nflav;iflav++) {
		cout << "c dentro measure a igauge=0 è" << endl;
		cout << c[index_c(imeas,nmeas,iflav,nflav,0)];
	}
*/
}	
...
/*
    cout << "Sono entrato in resample" << endl;
    cout << "block dentro resample è " << block << endl;
    cout << "ncopy dentro resample è " << ncopy << endl;
*/
...
/*
    cout << "sono uscito dal for di iconf dentro resample " << endl;
    cout << "alla fine di resample, ma sempre dentro, block=" << block << endl;
    cout << "alla fine di resample, ma sempre dentro, ncopy=" << ncopy << endl;
*/
}
...
/*
//stampo di prova c_i
for(iblock=0;iblock<nblock;iblock++) cout << "c_i è " << c_i[index_jack_c(0,nmeas,0,nflav,iblock,nblock,0)]<<endl;
*/
    //Sommo su iblock
    for(i_gauge=0;i_gauge<2;i_gauge++)
	for(imeas=0;imeas<nmeas;imeas++) 
	    for(iflav=0;iflav<nflav;iflav++)
		for(iblock=0;iblock<nblock;iblock++){ 
		    //if((i_gauge==0)&&(iflav==0)&&(imeas==0))cout << "E_j prima della somma è " << E_j[index_c(0,nmeas,0,nflav,0)] << endl;
		    E_j[index_c(imeas,nmeas,iflav,nflav,i_gauge)]+=c_i[index_jack_c(imeas,nmeas,iblock,nblock,iflav,nflav,i_gauge)];
		    //if((i_gauge==0)&&(iflav==0)&&(imeas==0))cout << "E_j dopo la somma è " << E_j[index_c(0,nmeas,0,nflav,0)] << endl;
		}
...
//cout << "E_j vale " << E_j[index_c(0,nmeas,0,nflav,0)] << endl;
...
//cout << "L'errore vale " << err[index_c(0,nmeas,0,nflav,0)] << endl;
}
...
void calc_err_top(float err[], float top[], int nmeas, int nconf, int ncopy, int block) {
    ...
    //Creo un singolo top_i. Va messo dentro un for. Forse ci vorranno anche due funzioni di resample, per via del numero di indici
    cout << "Sto per entrare nel ciclo dei blocchi dentro calc_err_top" << endl;
    cout << "nconf=" << nconf << " block=" << block << " nblock=" << nblock << endl;
    for(iblock=0;iblock<nblock;iblock++) {
	resample_top(top_i,iblock*block,top,nmeas,nconf,ncopy,block);	
	cout << "sono uscito da resample" << endl;
//	cout << "sono a iblock=" << iblock << endl;


	//Faccio una stampa di prova per vedere se funziona
	cout << "nconf=" << nconf << " block=" << block << endl;
	cout << "nconf-block=" << nconf-block << endl;
	cout << "ncopy è " << ncopy << endl;
/*
	for(iconf=0;iconf<(nconf-block);iconf++) { 	//iconf scorre il vettore sample, i_res quello resampled 
	    for(i_gauge=0;i_gauge<2;i_gauge++) {
		for(icopy=0;icopy<ncopy;icopy++) {
		    for(imeas=0;imeas<nmeas;imeas++) {
			cout << "indici: imeas=" << imeas << " " << "i_gauge=" << i_gauge << " " << "icopy=" << icopy << " " << "iconf=" << iconf << endl;
			cout << "top_i è " << top_i[index_q(imeas,nmeas,icopy,ncopy,iconf,nconf-block,i_gauge)] << endl;
		    }
		}
	    }
	}
	cout << "sono uscito dal for di prova" << endl;
*/
...
/*
	//for di prova per vedere se funziona la media
	cout << "La media per iblock=" << iblock << " è " << endl;
	for(i_gauge=0;i_gauge<2;i_gauge++) {
	    for(imeas=0;imeas<nmeas;imeas++) {
		    cout << "indici: imeas=" << imeas << " " << "i_gauge=" << i_gauge << " " << endl;
		    cout << "top_med è " << top_med[index_q_med(imeas,nmeas,i_gauge)] << endl;
	    }
	}
*/
	for(i_gauge=0;i_gauge<2;i_gauge++)
...
/*
    //stampa di prova per l'errore. Funziona
    cout << "L'errore per la plaq" << endl;
    for(i_gauge=0;i_gauge<2;i_gauge++)
	for(imeas=0;imeas<nmeas;imeas++) {
	    cout << "indici: imeas=" << imeas << " " << "i_gauge=" << i_gauge << " " << endl;
	    cout << err[index_q_med(imeas,nmeas,i_gauge)] << endl;
	}
	*/
}
...
void calc_err_spin(float err[], float spin[], int nmeas, int nconf, int ncopy, int nop, int nflav, int block) {
    ...
    for(iblock=0;iblock<nblock;iblock++) {
/*
	cout << "ncopy prima di resample_spin " << ncopy << endl;
	cout << "block prima di resample_spin " << block << endl;
	cout << "nflav prima di resample_spin " << nflav << endl;
	cout << "nop prima di resample_spin " << nop << endl;
*/
	resample_spin(spin_i,iblock*block,spin,nmeas,nconf,ncopy,nop,nflav,block);	
/*
	cout << "block all'uscita di resample_spin " << block << endl; 
	cout << "ncopy all'uscita di resample_spin è " << ncopy << endl;
	cout << "nflav dopo di resample_spin " << nflav << endl;
	cout << "nop dopo di resample_spin " << nop << endl;
*/

/*
//Faccio una stampa di prova per vedere se funziona il resample
	cout << "nconf=" << nconf << " block=" << block << endl;
	cout << "nconf-block=" << nconf-block << endl;
	cout << "ncopy è " << ncopy << endl;
	cout << "siamo a iblock = " << iblock << endl;
	...
	for(iconf=0;iconf<(nconf-block);iconf++)  	//iconf scorre il vettore sample, i_res quello resampled 
		...
		for(icopy=0;icopy<ncopy;icopy++) 
			for(iop=0;iop<nop;iop++) 
			    for(iflav=0;iflav<nflav;iflav++)
				for(imeas=0;imeas<nmeas;imeas++) {
				    cout << "indici: imeas=" << imeas << " " << "i_gauge=" << i_gauge << " " << "icopy=" << icopy << " " << "iconf=" << iconf << " iflav=" << iflav << " iop=" << iop << endl;
				    cout << "spin_i è " << spin_i[index_tens(imeas,nmeas,icopy,ncopy,iconf,nconf-block,iop,nop,iflav,nflav,i_gauge)] << endl;
		    }
	cout << "sono uscito dal for di prova" << endl;
*/
	//Secondo me così funziona, con quel valore di nconf
	//Metto a zero la media 
...
/*
	...
	for(i_gauge=0;i_gauge<2;i_gauge++) 
		...
		for(iflav=0;iflav<nflav;iflav++)
		    for(imeas=0;imeas<nmeas;imeas++) {
			cout << "indici: imeas=" << imeas << " " << "i_gauge=" << i_gauge << " iflav=" << iflav << " iop=" << iop <<endl;
			cout << "spin_med è " << spin_med[index_tens_med(imeas,nmeas,iop,nop,iflav,nflav,i_gauge)] << endl;
		    }
*/
	//Riempio spin_med_i
	for(i_gauge=0;i_gauge<2;i_gauge++)
...
/*
    //stampa di prova per l'errore. 
    cout << "L'errore spin " << endl;
    for(i_gauge=0;i_gauge<2;i_gauge++)
	for(iop=0;iop<nop;iop++)
	    for(iflav=0;iflav<nflav;iflav++)
		for(imeas=0;imeas<nmeas;imeas++) {
		    cout << "indici: imeas=" << imeas << " i_gauge=" << i_gauge << " iop=" <<iop << " iflav=" << iflav << endl;
		    cout << err[index_tens_med(imeas,nmeas,iop,nop,iflav,nflav,i_gauge)] << endl;
		}
*/
}
...
/*
//Faccio una stampa di prova per vedere se funziona il resample e il measure
	cout << "siamo dopo resample, a iblock = " << iblock << endl;

	for(iconf=0;iconf<(nconf-block);iconf++)  	//iconf scorre il vettore sample, i_res quello resampled 
		for(icopy=0;icopy<ncopy;icopy++) { 
			cout << "sono a imeas 0 e iconf " << iconf;
			cout << "spin_re_i è " << spin_re_i[index_tens(0,nmeas,icopy,ncopy,iconf,nconf-block,2,nop,0,nflav,0)] << endl;
			cout << "spin_im_i è " << spin_im_i[index_tens(0,nmeas,icopy,ncopy,iconf,nconf-block,2,nop,0,nflav,0)] << endl;
			cout << "spin_q_re_i è " << spin_q_re_i[index_tens(0,nmeas,icopy,ncopy,iconf,nconf-block,3,nop,0,nflav,0)] << endl;
			cout << "spin_q_im_i è " << spin_q_im_i[index_tens(0,nmeas,icopy,ncopy,iconf,nconf-block,3,nop,0,nflav,0)] << endl;
			cout << "top_i è " << top_i[index_q(0,nmeas,icopy,ncopy,iconf,nconf-block,0)] << endl;
		    }
	cout << "siamo a dopo measure" << endl;
	    cout << "spin_re_med è " << spin_re_med[index_tens_med(0,nmeas,2,nop,0,nflav,0)] << endl;
	    cout << "spin_im_med è " << spin_im_med[index_tens_med(0,nmeas,2,nop,0,nflav,0)] << endl;
	    cout << "spin_q_re_med è " << spin_q_re_med[index_tens_med(0,nmeas,3,nop,0,nflav,0)] << endl;
	    cout << "spin_q_im_med è " << spin_q_im_med[index_tens_med(0,nmeas,3,nop,0,nflav,0)] << endl;
	    cout << "top_med è " << top_med[index_q_med(0,nmeas,0)] << endl;
	    cout << "c_re_med è " << c_re_med[index_c(0,nmeas,0,nflav,0)] << endl;
	    cout << "c_im_med è " << c_im_med[index_c(0,nmeas,0,nflav,0)] << endl;
...
//stampo di prova c_i
for (iblock=0;iblock<nblock;iblock++) {
	cout <<"c_re_med_i è " << c_re_med_i[index_jack_c(0,nmeas,0,nflav,iblock,nblock,0)] << endl;
	cout <<"c_im_med_i è " << c_im_med_i[index_jack_c(0,nmeas,0,nflav,iblock,nblock,0)] << endl;
}
*/
    //Calcolo l'errore 
...
//    read_par(&nmeas, &ncopy, &nconf, &nop);	//Leggo i parametri di ingresso nell'input-file. Se non va darli in input al programma
    ...
    //cout << "Sto per riempire i vettori di dati" << endl;
    ...
    //cout << "Ho riempito i vettori di dati" << endl;
    ...
    //cout << "Ho misurato la media sulle configurazioni delle grandezze topologiche" << endl;
    ...
    //cout << "Ho misurato la media sulle configurazioni delle grandezze spinoriale" << endl;
/*
    for(int imeas=0;imeas<nmeas;imeas++)
	for(int i_gauge=0;i_gauge<2;i_gauge++) //smooth=0, ferm=1
	    for(int iflav=0;iflav<nflav;iflav++) {
		cout << "indici prima di measure imeas=" << imeas << "\t" << "i_gauge=" << i_gauge << "\t" << "iflav=" << iflav << endl;
		cout << "q2 prima di measure è " << q2_med[index_q_med(imeas,nmeas,i_gauge)] << endl;
		cout << "spin_q_re iop=3 è " << tens_dens_re_med[index_tens_med(imeas,nmeas,3,nop,iflav,nflav,i_gauge)] << endl;
		cout << "spin_q_im iop=3 è " << tens_dens_im_med[index_tens_med(imeas,nmeas,3,nop,iflav,nflav,i_gauge)] << endl;
		cout << "spin_re iop=2 è " << tens_re_med[index_tens_med(imeas,nmeas,2,nop,iflav,nflav,i_gauge)] << endl;
		cout << "spin_im iop=2 è " << tens_im_med[index_tens_med(imeas,nmeas,2,nop,iflav,nflav,i_gauge)] << endl;
	    }
 */  
    ...
    cout << "block nel main è " << block << endl;
    cout << "ncopy nel main è " << ncopy << endl;
    calc_err_top(plaq_err,plaq,nmeas,nconf,ncopy,block);	
    //cout << "ho calcolato l'errore col jackknife della placchetta" << endl;
   ...
   // cout << "Ho misurato l'errore per tens_re" << endl;
