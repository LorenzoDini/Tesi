//Questa è la versione in cui metto il valore dei block variabile. Manca la modifica 3 degli appunti. 
//probabilmente è finito così e funzionante. Però è la versione prima delle modifiche a nissa con Sanfo e il wf fermionico. Parto da questa per scrivere il nuovo measure
...
int main () {
  ...
  //Attenzione: devo mettere nel read di buttare le conf per fare in modo che siano multiple di block
    int nmeas, ncopy, nconf, nop, nflav=3, block;	//Valore di prova per block
